...
public class PotionTargetCrossbow : PotionTargetAbstract
{
    [Header("Size Effect")]
    [Range(0.1f, 1f)]
    ...
    public GameObject coldArrow;
    public GameObject hotArrow;
    private LaunchProjectile shootScript;
    private float baseShootSpeed;

    public override void Start()
    {
        base.Start();
        shootScript = GetComponent<LaunchProjectile>();
        baseShootSpeed = shootScript.launchSpeed;
    }
    ...
    protected override void OnPotionEffectsAdded(PotionEffects.Effects effects)
    {
        ...
        if (effects.HasFlag(PotionEffects.Effects.Light))
        {
            if (objectRoot.XRGrabInteractable)
            {
                ...
                shootScript.launchSpeed = baseShootSpeed * lightMultiplier;
            }
        }
        if (effects.HasFlag(PotionEffects.Effects.Heavy))
        {
            if (objectRoot.XRGrabInteractable)
            {
                ...
                shootScript.launchSpeed = baseShootSpeed * heavyMultiplier;
            }
        }
        ...
        if (effects.HasFlag(PotionEffects.Effects.Cold))
        {
            if (objectRoot.collider && objectRoot.collider.material)
            {
                ...
                shootScript.projectilePrefab = coldArrow;
            }
        }
        if (effects.HasFlag(PotionEffects.Effects.Hot))
        {
            if (objectRoot.collider && objectRoot.collider.material)
            {
                ...
                shootScript.projectilePrefab = hotArrow;
            }
        }
    }
    protected override void OnPotionEffectsRemoved(PotionEffects.Effects effects)
    {
        ...
        if (effects.HasFlag(PotionEffects.Effects.Light))
        {
            ...
            {
                objectRoot.XRGrabInteractable.throwVelocityScale /= lightMultiplier;
                shootScript.launchSpeed = baseShootSpeed;
            }
        }
        if (effects.HasFlag(PotionEffects.Effects.Heavy))
        {
        ...
        if (effects.HasFlag(PotionEffects.Effects.Cold))
        {
            if (objectRoot.collider && objectRoot.collider.material)
            {
                ...
                shootScript.projectilePrefab = defaultArrow;
            }
        }
        if (effects.HasFlag(PotionEffects.Effects.Hot))
        {
